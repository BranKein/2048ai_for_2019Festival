...
def checking_game_over(mat):  # 게임이 끝났는지 확인
    ...
    for i in range(4):
        for j in range(3):
            if mat[i][j] == mat[i][j + 1] and mat[i][j] != 0:
                score += mat[i][j] * 2
                score_power += (mat[i][j] * 2) * (mat[i][j] * 2)
                mat[i][j] *= 2
                mat[i][j + 1] = 0
                done = True # 합쳤다
    return (mat, score,score_power, done)
...
def up(game):
    ...
    game = transpose(game)
    return (game, score, score_power, done)
