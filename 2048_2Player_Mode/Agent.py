...
class Brain:
        ...
        def make_desicion(self, grid):
            ...
            input_x_best = None
            for m in range(4):
                mat, done, s = ExpectedMax.move_action(g, m)
                input_x = []
                if done == True:
                    input_x = self.make_input(grid)
                    v = self.model.predict(np.array([input_x]), batch_size=1)[0][0]
...
class Agent:
        ...
        def Act(self, s):
                if (random.random() < self.epsilon_game):
                        return random.randint(0, self.NbrActions-1)                                             # Explore
                else:
                        return numpy.argmax(self.brain.make_desicion(s))                                   # Exploit Brain best Prediction
        '''
        입실론-탐욕 정책
